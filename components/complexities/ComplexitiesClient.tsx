...
export function ComplexitiesClient({
  ...
  const handleEdit = async (e: FormEvent) => {
    ...
    setLoading(true)
    try {
      const response = await fetch(withBasePath(`/api/complexities/${editingComplexity.id}`), {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(formData),
      })
  ...
  const handleDelete = async () => {
    ...
    setLoading(true)
    try {
      const response = await fetch(withBasePath(`/api/complexities/${deletingComplexity.id}`), {
        method: 'DELETE',
      })
      const data = await response.json()
